...
def introduce_errors(row):
    ...
    if random.random() < 0.1:  # 10% chance to introduce a null value
        row['Email'] = None
    if random.random() < 0.05:  # 5% chance to add strange characters to names
        row['Customer Name'] = row['Customer Name'] + ' @#$'
    if random.random() < 0.1:  # 10% chance for incorrect phone number
        row['Phone'] = '12345'
    if random.random() < 0.05:  # 5% chance for invalid total price
...
def generate_customer_data(row_count):
    ...
    for _ in range(row_count):
        ...
        row = introduce_errors(row)  # Introduce errors
        data.append(row)
